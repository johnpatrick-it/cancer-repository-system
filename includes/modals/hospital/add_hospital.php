...
if (isset($_POST['add_hospital'])) {
    ...
    $hospitalInsertQuery = "INSERT INTO hospital_general_information (
            admin_id,
            hospital_name,
            hospital_level,
            type_of_institution,
    ...
    if (isset($_POST['hospital-equipment']) && is_array($_POST['hospital-equipment'])) {
        $equipmentArray = $_POST['hospital-equipment'];

        foreach ($equipmentArray as $equipmentName) {
            $equipmentInsertQuery = "INSERT INTO hospital_equipment (
                hospital_id,
                admin_id,
                hospital_name,
                hospital_level,
            ...
            mysqli_query($db, $equipmentInsertQuery);
        }
    }
}
            ...
            <div class="modal-body">
                <form method="POST" enctype="multipart/form-data" autocomplete="off">
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-3">
                            ...
                            <div class="input-group">
                                <input name="hospital-equipment" class="form-control" type="text"
                    ...
                    <div class="form-row" id="equipmentContainer">
                        <!-- new equipment will be added here -->
                    </div>
            </div>
...
$(document).ready(function() {
    // Add new equipment row on button click
    $('.add-equipment-btn').click(function() {
        var newEquipmentField = '<div class="form-group col-md-3">' +
            '<label for="hospital-equipment">Oncologists Medical Equipment</label>' +
            '<div class="input-group">' +
            '<input name="hospital-equipment[]" class="form-control" type="text" placeholder="Medical Equipment" required>' +
            '<div class="input-group-append">' +
            '<button type="button" class="btn btn-danger remove-equipment-btn">-</button>' +
            '</div>' +
            '</div>' +
            '</div>';

        $('#equipmentContainer').append(newEquipmentField);
    });

    // Remove equipment row on button click
